...
// Environment checks
var on_ground = place_meeting(x, y + 1, oTopGround);
var wall_left = place_meeting(x - 1, y, oTopGround);
var wall_right = place_meeting(x + 1, y, oTopGround);
...
if (!place_meeting(x + hsp, y, oTopGround))
{
    ...
    while (!place_meeting(x + sign(hsp), y, oTopGround)) 
	{
        x += sign(hsp);
    }
    hsp = 0;
...
if (!place_meeting(x, y + vsp, oTopGround)) 
{
    ...
    while (!place_meeting(x, y + sign(vsp), oTopGround)) 
	{
        y += sign(vsp);
    }
    vsp = 0;
